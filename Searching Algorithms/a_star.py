...
def a_star(graph, start, goal):
    ...
    while not queue.empty():
        curHeuristicPlusCost, currentNode = queue.get()
        ...
        for neighbor in graph.neighbors(currentNode):
            if neighbor not in visited:
                prevCost = curHeuristicPlusCost - graph.get_heuristics(currentNode)
                aggregateCost = prevCost + graph.get_cost(currentNode, neighbor)
                queue.put((aggregateCost + graph.get_heuristics(neighbor), neighbor))
